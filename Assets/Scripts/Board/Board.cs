...
public class Board
{
    ...
    internal void Fill()
    {
        ...
        if (totalCells % 3 != 0)
        {
            Debug.LogWarning($"Board size ({boardSizeX}x{boardSizeY} = {totalCells}) is not divisible by 3. Falling back to legacy fill.");
            FillLegacy();
            return;
        }
        ...
        {
            var budgets = CreateTypeBudgets(totalCells);
            if (TryBuildLayout(budgets, out var layout))
            {
                ApplyLayout(layout);
                return;
            }
        ...
        Debug.LogWarning("Unable to generate an initial board that satisfies the triple distribution constraint. Falling back to legacy fill.");
        FillLegacy();
    }
    ...
    private Dictionary<NormalItem.eNormalType, int> CreateTypeBudgets(int totalCells)
    {
        var types = Enum.GetValues(typeof(NormalItem.eNormalType)).Cast<NormalItem.eNormalType>().ToList();
        var budgets = types.ToDictionary(t => t, _ => 0);

        int tripleCount = totalCells / 3;
        var shuffledTypes = types.OrderBy(_ => UnityEngine.Random.value).ToList();
        int typeIndex = 0;

        for (int i = 0; i < tripleCount; i++)
        {
            if (typeIndex >= shuffledTypes.Count)
            {
                shuffledTypes = types.OrderBy(_ => UnityEngine.Random.value).ToList();
                typeIndex = 0;
            }

            var selectedType = shuffledTypes[typeIndex];
            budgets[selectedType] += 3;
            typeIndex++;
        }
    ...
    private bool TryBuildLayout(Dictionary<NormalItem.eNormalType, int> budgets, out NormalItem.eNormalType[,] layout)
    {
        layout = null;

        var budgetsCopy = new Dictionary<NormalItem.eNormalType, int>(budgets);
        var tentative = new NormalItem.eNormalType?[boardSizeX, boardSizeY];

        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                var blocked = new HashSet<NormalItem.eNormalType>();

                if (y > 0 && tentative[x, y - 1].HasValue)
                {
                    blocked.Add(tentative[x, y - 1].Value);
                }

                if (x > 0 && tentative[x - 1, y].HasValue)
                {
                    blocked.Add(tentative[x - 1, y].Value);
                }

                var candidateKVs = budgetsCopy
                    .Where(kv => kv.Value > 0 && !blocked.Contains(kv.Key))
                    .OrderByDescending(kv => kv.Value)
                    .ToList();

                if (candidateKVs.Count == 0)
                {
                    return false;
                }

                int highest = candidateKVs[0].Value;
                var topCandidates = candidateKVs
                    .Where(kv => kv.Value == highest)
                    .Select(kv => kv.Key)
                    .ToList();

                var chosen = topCandidates[UnityEngine.Random.Range(0, topCandidates.Count)];

                tentative[x, y] = chosen;
                budgetsCopy[chosen]--;
            }
        }

        layout = new NormalItem.eNormalType[boardSizeX, boardSizeY];
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                layout[x, y] = tentative[x, y].Value;
            }
        }
    ...
    private void ApplyLayout(NormalItem.eNormalType[,] layout)
    {
        ClearBoardItems();
